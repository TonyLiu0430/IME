...
DebugSink& DebugSink::instance() {
    static DebugSink s;
    return s;
}

void DebugSink::connect(const char* host, unsigned short port) {
    if (_sock != INVALID_SOCKET) return;  // 已連線
...
void DebugSink::send(const wchar_t* tag, const std::wstring& text) {
...
void DebugSink::send(const wchar_t* tag, wchar_t ch) {
...
bool DebugSink::isConnected() const {
...
std::string DebugSink::toUtf8(const std::wstring& ws) {
    if (ws.empty()) return {};
